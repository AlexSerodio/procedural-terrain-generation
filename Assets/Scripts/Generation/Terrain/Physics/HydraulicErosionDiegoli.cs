...
namespace Generation.Terrain.Physics.Erosion
{
    ...
    /// </remarks>
    /// </summary>
    public class HydraulicErosionDiegoli
    {
        public virtual void Erode(float[,] heightmap, float pour, float solubility, float evaporationFactor, int iterations)
        {
            float[,] water = SetWater(heightmap);

            for (int i = 0; i < iterations; i++)
                Erode(heightmap, water, pour, solubility, evaporationFactor);
        }

        public virtual void Erode(float[,] heightmap, float[,] water, float rainFactor, float solubility, float evaporationFactor)
        {
            PourWater(water, rainFactor);
            Dissolve(heightmap, water, solubility);
            WaterFlow(heightmap, water);
            DrainWater(heightmap, water, solubility, evaporationFactor);
        }

        private float[,] SetWater(float[,] heightmap)
        {
            int maxX = heightmap.GetLength(0);
            int maxY = heightmap.GetLength(1);
            
            float[,] water = new float[maxX, maxY];

            for (int x = 0; x < maxX; x++)
                for (int y = 0; y < maxY; y++)
                    water[x, y] = heightmap[x, y];
